async function throttlePromises() {
    const asyncTasks = [
        ...
        () => new Promise(resolve => setTimeout(() => { console.log("Task 3 done"); resolve("Task 3 done");}, 1500)),
        () => new Promise(resolve => setTimeout(() => { console.log("Task 4 done"); resolve("Task 4 done");}, 500)),
    ];

    async function throttle(task, limit) {
        const results = [];
        const executing = [];
